...
// TODO: Pass string xml first into a xml processor to convert it to string of just the contents
// TODO: Pass string xml of second into a xml processor to convert it to a string of just the contents
...
void CompareController::SubmitCompare(Query query, std::string firstDate, std::string secondDate) {
    _status = LOADING;
    try {
        ...
        std::string outputFirstDate = RegulationService::ProcessCompareResponse(contentFirstDateJson);
        std::string outputSecondDate = RegulationService::ProcessCompareResponse(contentSecondDateJson);
        _status = SUCCESS;
